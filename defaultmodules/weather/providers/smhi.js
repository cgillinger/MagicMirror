...
class SMHIProvider {
	...
	constructor (config) {
		...
		if (!Object.keys(SMHIProvider.PRECIP_VALUE_MAP).includes(this.config.precipitationValue)) {
	...
	#convertWeatherDataToObject (weatherData, coordinates) {
		...
		const precipParamName = SMHIProvider.PRECIP_VALUE_MAP[this.config.precipitationValue];
		...
		// SNOW1gv1 pcat values: 0=none, 1=snow, 2=snow+rain, 5=freezing rain,
		// 6=freezing drizzle, 11=drizzle/light rain
		switch (pcat) {
			case 1: // Snow
			...
			case 2: // Snow and rain (50/50 split)
				...
				current.precipitationAmount = precipitationValue;
				break;
			case 3: // Rain (legacy, may still appear)
			case 4: // Drizzle (legacy, may still appear)
			case 5: // Freezing rain
			case 6: // Freezing drizzle
			case 11: // Drizzle / light rain (new in SNOW1gv1)
